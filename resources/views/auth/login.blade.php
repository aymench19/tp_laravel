...
<form method="POST" action="{{ route('login') }}" class="w-full max-w-sm mx-auto rounded-lg border shadow-md p-5 mb-5">
    ...
    <div class="mb-4">
        ...
        <input id="email" type="email" name="email" class="shadow border rounded w-full p-2" value="{{ old('email') }}" autocomplete="email" placeholder="Votre email" autofocus required>
        @error('email')
    ...
    <div class="mb-4">
        ...
        <input id="password" type="password" name="password" class="shadow border rounded w-full p-2" value="{{ old('password') }}" autocomplete="current-password" placeholder="Votre mot de passe" required>
        @error('password')
